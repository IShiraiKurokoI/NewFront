    ...
    <el-row>
      ...
      <el-col :span="7">
          医生姓名：<el-input disabled v-model="dName" class="orderInput"></el-input>
              ...
              <el-col :span="3">
          <el-button type="success" @click="openAdvice">诊断/意见</el-button>
      </el-col>
    </el-row>
      <!-- 药物表格 -->
      <el-row>
      ...
      <el-table :data="drugData" stripe border>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="success" size="mini" @click="addDrug(scope.row.drId)">增加</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
          ...
          <el-table-column label="名称" prop="drName"></el-table-column>
           <el-table-column label="单价" prop="drPrice"></el-table-column>
            <el-table-column label="数量" prop="drNum"></el-table-column>
             ...
             <el-table-column label="操作">
               <template slot-scope="scope">
                 <el-button type="danger" size="mini" @click="deleteDrug(scope.row.drId)">移除</el-button>
               </template>
             </el-table-column>
        </el-table>
          ...
          <el-table stripe border :data="checkData">
            <el-table-column label="编号" prop="chId"></el-table-column>
             <el-table-column label="项目" prop="chName"></el-table-column>
              <el-table-column label="价格" prop="chDescription"></el-table-column>
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <el-button type="success" size="mini" @click="addCheck(scope.row.chId)">增加</el-button>
                </template>
              </el-table-column>
          </el-table>
                <!-- 分页 -->
        ...
        <el-table stripe border class="rigthTable" :data="checkBuyData">
          <el-table-column label="编号" prop="chId"></el-table-column>
             <el-table-column label="项目" prop="chName"></el-table-column>
              ...
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <el-button type="danger" size="mini" @click="deleteCheck(scope.row.chId)">移除</el-button>
                </template>
              </el-table-column>
              </el-table>
    ...
    dataPackage(){
      ...
      for(let i=0;i<this.drugBuyData.length;i++){
       ...
       for(let i=0;i<this.checkBuyData.length;i++){
        oCheck += this.checkBuyData[i].chName+"*"+this.checkBuyData[i].chDescription+"(元) ";
      }
    ...
    deleteCheck(chId){
      for(let i=0;i<this.checkBuyData.length;i++){
        if(this.checkBuyData[i].chId === chId){
          this.checkTotalPrice -= this.checkBuyData[i].chDescription;//药物价格总计
      ...
      .then(res => {
        ...
        //后端传过来的是对象，表格绑定的数组
           this.checkBuyData.push(
          {
            ...
            chDescription: res.data.data.chDescription,
            chName:res.data.data.chName,
          }
          )
           this.checkTotalPrice += res.data.data.chDescription;//药物价格总计
