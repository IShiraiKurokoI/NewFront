<template>
  <div>
    <el-card>
      <el-table :data="orderData" stripe style="width: 100%" border>
        ...
        <el-table-column label="报告单">
          <template slot-scope="scope">
            <el-button type="success" size="mini" @click="seeReport(scope.row.oId)" v-if="scope.row.oState === 1 && scope.row.oPriceState === 1">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="用户评价" :visible.sync="starVisible">
      <div><h3>请对工号：{{dId}}&nbsp;医生：{{dName}}&nbsp;进行评价</h3></div>
      <div>
            <el-rate
  v-model="star"
  show-text>
...
<div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="starClick"
          >确 定</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    //查看报告单
    seeReport(id){    
        window.location.href = 'http://localhost:8081/patient/pdf?oId='+id; 
    },
    //点击缴费按钮
    priceClick(oId, dId) {
      request
