...
<el-button size="small" type="success" @click="exportDoctors">一键导出</el-button></el-col>
    ...
    <el-table :data="doctorData" stripe style="width: 100%" border>
      ...
      <el-table-column prop="dGender" label="性别" width="40">
      </el-table-column>
      ...
      <el-table-column label="操作" width="140">
        <template slot-scope="scope">
          <el-button size="mini" type="success" icon="el-icon-edit" @click="modifyDialog(scope.row.dId)"></el-button>
          <el-button
            size="mini"
            type="danger"
            icon="el-icon-delete"
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        dName: [
          ...
          { min:2,max:5, message: "账号必须是2到5个字符", trigger:"blur" },
        ],
         dPost: [
  ...
  methods: {
    //导出医生信息
    exportDoctors(){
      window.location.href = 'http://localhost:8081/doctor/downloadExcel';
    },
    //文件上传
    handleProgress(){
    ...
    //删除医生操作
    deleteDoctor(id){
      request.get("admin/deleteDoctor", {
        params: {
          dId: id
        }
      })
